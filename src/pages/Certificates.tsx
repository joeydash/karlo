...
import { ArrowLeft, Award, Search, X, Plus, Edit, Trash2, ChevronDown, Users } from 'lucide-react';
...
const Certificates: React.FC = () => {
  ...
  const { certificates, isLoading, error, fetchCertificates, fetchMemberCertificates, deleteCertificate } = useCertificate(true);
  ...
  }, [selectedMemberId, currentUser?.id, currentOrganization?.id, isCurrentUserAdmin]);
  ...
  const filteredCertificates = certificates.filter(certificate =>
    ...
    (certificate.designation && certificate.designation.toLowerCase().includes(searchTerm.toLowerCase()))
  );
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
            ...
            <div className="flex items-center py-3 sm:py-4">
              ...
              <div className="flex items-center space-x-2 sm:space-x-3 min-w-0">
                ...
                <div className="min-w-0">
                  <h1 className="text-base sm:text-xl font-bold text-gray-900 dark:text-white truncate">Certificates</h1>
                  <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-300 hidden sm:block">Your professional certificates</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
          <div className="text-center py-8 sm:py-12 px-4">
            ...
            <h3 className="text-base sm:text-lg font-medium text-gray-900 dark:text-white mb-2">Failed to load certificates</h3>
            <p className="text-sm sm:text-base text-gray-600 dark:text-gray-300 mb-4 sm:mb-6">{error}</p>
            <button
              onClick={() => navigate('/dashboard')}
          ...
          <div className="flex items-center justify-between py-3 sm:py-4">
            <div className="flex items-center min-w-0 flex-1">
              ...
              <div className="flex items-center space-x-2 sm:space-x-3 min-w-0">
                ...
                <div className="min-w-0">
                  <h1 className="text-base sm:text-xl font-bold text-gray-900 dark:text-white truncate">Certificates</h1>
                  <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-300 hidden sm:block">Professional certificates and achievements</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4 sm:space-y-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
              <div className="min-w-0">
                <h2 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">
                  {isCurrentUserAdmin ? 'Manage Certificates' : 'My Certificates'}
                </h2>
                <p className="text-sm sm:text-base text-gray-600 dark:text-gray-300">
                  {searchTerm ? (
                    <>
                      {filteredCertificates.length} of {certificates.length} certificate{certificates.length !== 1 ? 's' : ''}
                      {filteredCertificates.length > 0 && ` matching "${searchTerm}"`}
                    </>
                  ) : (
                  ...
                  <>
                    <div className="relative">
                      ...
                      >
                        ...
                        <span>{selectedMember?.auth_fullname.fullname || 'Select Member'}</span>
                            ...
                            {filteredMembers.map((member) => (
                              ...
                              >
                                <div className="flex-1">
                                  <p className="text-sm font-medium">{member.auth_fullname.fullname}</p>
                                </div>
                              </button>
                            ))}
            ...
            {filteredCertificates.length === 0 && !searchTerm ? (
              <div className="text-center py-8 sm:py-12 px-4">
                ...
                <h3 className="text-base sm:text-lg font-medium text-gray-900 dark:text-white mb-2">No certificates yet</h3>
                <p className="text-sm sm:text-base text-gray-600 dark:text-gray-300">
                  {isCurrentUserAdmin ? 'Create certificates for team members' : 'Your professional certificates will appear here'}
                </p>
              </div>
            ) : filteredCertificates.length === 0 && searchTerm ? (
              <div className="text-center py-8 sm:py-12 px-4">
                <Search className="h-12 w-12 sm:h-16 sm:w-16 text-gray-300 dark:text-gray-600 mx-auto mb-3 sm:mb-4" />
                <h3 className="text-base sm:text-lg font-medium text-gray-900 dark:text-white mb-2">No certificates found</h3>
                <p className="text-sm sm:text-base text-gray-600 dark:text-gray-300 mb-4 sm:mb-6">No certificates match "{searchTerm}"</p>
                <button
                  onClick={() => setSearchTerm('')}
                        ...
                        >
                          ...
                          <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400">
                              {calculateDuration(certificate.from_date, certificate.to_date)}
                            </span>
                          </td>
                          {isCurrentUserAdmin && (
                            <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                              ...
                              <button
                                onClick={(e) => handleDeleteClick(e, certificate.id)}
                ...
                {filteredCertificates.map((certificate) => (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-3 text-xs">
                      <div>
                        <p className="text-gray-500 dark:text-gray-400 mb-1">From Date</p>
                        <p className="text-gray-900 dark:text-white font-medium">
                          {new Date(certificate.from_date).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}
                        </p>
                      </div>
                      <div>
                        <p className="text-gray-500 dark:text-gray-400 mb-1">To Date</p>
                        <p className="text-gray-900 dark:text-white font-medium">
                          {new Date(certificate.to_date).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}
                        </p>
                      </div>
                    ...
                    <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
                      <p className="text-xs text-gray-500 dark:text-gray-400">
                        Added {new Date(certificate.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                      </p>
                    </div>
                  </div>
                ))}
