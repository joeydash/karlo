...
import { ArrowLeft, Search, Filter, ChevronDown, Clock, Edit, Trash2, X, CheckCircle, ChevronLeft, ChevronRight, Users, Plus, XCircle, AlertCircle } from 'lucide-react';
...
const OOTOH: React.FC = () => {
  ...
  const { ootohRecords, total, isLoading, error, fetchOOTOHRecords, fetchAllOOTOHRecords, updateOOTOH, deleteOOTOH, recordOOTOH, approveOOTOH, rejectOOTOH } = useOOTOH();
  ...
  useEffect(() => {
    ...
    if (isCurrentUserAdmin) {
      if (selectedMemberId === '') {
        ...
        fetchOOTOHRecords(currentOrganization.id, currentPage, itemsPerPage, selectedMemberId);
      }
    } else {
      fetchOOTOHRecords(currentOrganization.id, currentPage, itemsPerPage, currentUser?.id);
    }
  ...
  const handleRecordOOTOH = async (startTime: string, startDate: string, endTime: string | null, endDate: string | null, workDone: string | null, targetMemberId?: string) => {
  ...
  const filteredOOTOH = ootohRecords.filter(record => {
    const matchesSearch = selectedMemberId === ''
      ? record.auth_fullname?.fullname?.toLowerCase().includes(searchTerm.toLowerCase()) || false
      : true;
    ...
    const matchesYear = selectedYear === 'all' || recordYear === parseInt(selectedYear);
  ...
  const availableYears = Array.from(new Set(ootohRecords.map(record => new Date(record.start_date).getFullYear())))
    .sort((a, b) => b - a);
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
          ...
          <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8">
            <div className="flex items-center py-3 sm:py-4">
              ...
              <div className="flex items-center space-x-2 sm:space-x-3">
                ...
                <div>
                  <h1 className="text-base sm:text-xl font-bold text-gray-900 dark:text-white">Out of Office Hours</h1>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">Failed to load OOTOH records</h3>
              ...
              <div className="flex items-center w-full sm:w-auto">
                ...
                <div className="flex items-center space-x-2 sm:space-x-3 min-w-0 flex-1">
                  ...
                  <div className="min-w-0">
                    <h1 className="text-base sm:text-xl font-bold text-gray-900 dark:text-white truncate">Out Of The Office Hours</h1>
                    <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-300 hidden sm:block">Track office entry and exit times</p>
                  </div>
                </div>
              </div>
                ...
                >
                  ...
                  {showMemberDropdown && (
                    <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-600 py-2 z-20 max-h-60 overflow-y-auto">
                      ...
                      {(!isCurrentUserAdmin || !memberSearchTerm || 'all members'.includes(memberSearchTerm.toLowerCase())) && (
                        <button
                          onClick={() => {
                            setSelectedMemberId('');
                          ...
                          className={`w-full flex items-center justify-between px-4 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200 ${
                            selectedMemberId === '' ? 'text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-900/20' : 'text-gray-700 dark:text-gray-300'
                          }`}
                        >
                      ...
                      {filteredMembers.map((member) => (
                        <button
                          ...
                          className={`w-full flex items-center justify-between px-4 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200 ${
                            selectedMemberId === member.user_id ? 'text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-900/20' : 'text-gray-700 dark:text-gray-300'
                          }`}
                        >
                          <div className="flex items-center space-x-3">
                            {member.auth_fullname.dp ? (
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
              ...
              <div>
                <h2 className="text-lg sm:text-2xl font-bold text-gray-900 dark:text-white">
                  {(() => {
                    if (isCurrentUserAdmin && selectedMemberId === '') {
                      ...
                      const member = members.find(m => m.user_id === selectedMemberId);
                      return `${member?.auth_fullname.fullname || 'Member'}'s OOTOH Records`;
                    } else {
                ...
                <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-300">
                  {searchTerm || selectedMonth !== 'all' || selectedYear !== 'all' ? (
                    <>
                      {filteredOOTOH.length} of {ootohRecords.length} record{ootohRecords.length !== 1 ? 's' : ''}
                      {filteredOOTOH.length > 0 && searchTerm && ` matching "${searchTerm}"`}
                      {selectedMonth !== 'all' && ` in ${monthOptions.find(m => m.value === selectedMonth)?.label}`}
                      {selectedYear !== 'all' && ` for year ${selectedYear}`}
                    </>
                  ) : (
                    `${ootohRecords.length} record${ootohRecords.length !== 1 ? 's' : ''}`
                  )}
                </p>
              </div>
                        ...
                        <div className="absolute right-0 top-full mt-2 w-48 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-600 py-2 z-20 max-h-56 overflow-y-auto custom-scrollbar">
                          {monthOptions.map((option) => (
                            <button
                              ...
                              className={`w-full flex items-center justify-between px-4 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200 ${
                                selectedMonth === option.value ? 'text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-900/20' : 'text-gray-700 dark:text-gray-300'
                              }`}
                            >
                        ...
                        <div className="absolute right-0 top-full mt-2 w-48 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-600 py-2 z-20 max-h-56 overflow-y-auto custom-scrollbar">
                          {yearOptions.map((option) => (
                            <button
                              ...
                              className={`w-full flex items-center justify-between px-4 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200 ${
                                selectedYear === option.value ? 'text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-900/20' : 'text-gray-700 dark:text-gray-300'
                              }`}
                            >
                              <span>{option.label}</span>
          ...
          {isLoading && ootohRecords.length === 0 ? (
                ...
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                  <thead className="bg-gray-50 dark:bg-gray-700">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Entry Time</th>
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">No OOTOH records yet</h3>
              <p className="text-gray-600 dark:text-gray-300 mb-6">Record your first office hours to get started</p>
              {(!isCurrentUserAdmin || (isCurrentUserAdmin && selectedMemberId && selectedMemberId !== '')) && (
                <button
                  onClick={() => setShowRecordModal(true)}
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                      {paginatedRecords.map((record) => (
                        <tr key={record.id} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200">
                          ...
                          {isCurrentUserAdmin && selectedMemberId === '' && record.auth_fullname && (
                            <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                              <div className="flex items-center space-x-3">
                          ...
                          <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                            <div className="space-y-1">
                              <div className={`text-sm font-medium ${record.end_time ? 'text-gray-900 dark:text-white' : 'text-gray-400 dark:text-gray-500'}`}>
                          ...
                          <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                            <div className={`text-sm font-medium ${record.end_time ? 'text-orange-600 dark:text-orange-400' : 'text-gray-400 dark:text-gray-500'}`}>
                          ...
                          <td className="px-4 sm:px-6 py-4 max-w-xs">
                            <div className="text-sm text-gray-700 dark:text-gray-300 line-clamp-2">
                              {record.work_done || <span className="text-gray-400 dark:text-gray-500">No description</span>}
                            </div>
                          </td>
                          <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-left text-sm font-medium">
                            <div className="flex items-center space-x-2">
                              ...
                              {!record.end_time && (record.user_id === currentUser?.id || isCurrentUserAdmin) && (
                                <button
                                  onClick={() => handleUpdateOOTOH(record)}
                                  className="text-orange-600 dark:text-orange-400 hover:text-orange-900 dark:hover:text-orange-300 hover:bg-orange-50 dark:hover:bg-orange-900/20 p-2 rounded-lg transition-all duration-200"
                                  title="Add end time"
                                >
                                  <Edit className="h-4 w-4" />
                                </button>
                              )}
                              {(record.user_id === currentUser?.id || isCurrentUserAdmin) && (
                                <button
                                  onClick={() => handleDeleteOOTOH(record.id)}
                                  className="text-red-600 dark:text-red-400 hover:text-red-900 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-900/20 p-2 rounded-lg transition-all duration-200"
                                  title="Delete record"
                                >
                  ...
                  <div className="px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div className="flex items-center space-x-4">
                      <span className="text-sm text-gray-700 dark:text-gray-300">
                        ...
                        <span className="font-semibold">{Math.min(endIndex, filteredOOTOH.length)}</span> of{' '}
                        <span className="font-semibold">{filteredOOTOH.length}</span> results
                      </span>
                      <div className="flex items-center space-x-2">
                  ...
                  <div key={record.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4">
                    ...
                    <div className="space-y-2 mb-3">
                      <div className="flex items-center justify-between text-xs">
                        ...
                        <span className="font-medium text-gray-900 dark:text-white">{formatTime(record.start_time)}</span>
                      </div>
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-gray-600 dark:text-gray-400">End Time</span>
                        <div className="text-right">
                          <div className={`font-medium ${record.end_time ? 'text-gray-900 dark:text-white' : 'text-gray-400 dark:text-gray-500'}`}>
                            {formatTime(record.end_time)}
                          </div>
                          {record.end_date && record.end_date !== record.start_date && (
                      ...
                      <div className="flex items-center justify-between text-xs">
                        ...
                        <span className={`font-medium ${record.end_time ? 'text-orange-600 dark:text-orange-400' : 'text-gray-400 dark:text-gray-500'}`}>
                          {calculateDuration(record.start_time, record.end_time)}
                        </span>
                      </div>
                      <div className="flex items-center justify-between text-xs">
                      ...
                      {record.work_done && (
                        <div className="pt-2 border-t border-gray-100 dark:border-gray-700">
                          <p className="text-xs text-gray-600 dark:text-gray-400 mb-1 font-medium">Work Done:</p>
                          <p className="text-xs text-gray-700 dark:text-gray-300 whitespace-pre-wrap">{record.work_done}</p>
                        </div>
                      )}
                    </div>
                      ...
                      {(record.user_id === currentUser?.id || isCurrentUserAdmin) && (
                        <>
                          {!record.end_time && (
                            <button
                              onClick={() => handleUpdateOOTOH(record)}
                              className="flex-1 flex items-center justify-center space-x-1 px-3 py-2 text-xs font-medium text-orange-700 dark:text-orange-300 bg-orange-50 dark:bg-orange-900/20 hover:bg-orange-100 dark:hover:bg-orange-900/30 rounded-lg transition-all duration-200"
                            >
                ...
                {filteredOOTOH.length > 0 && (
                  <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 space-y-3">
                    <div className="flex items-center justify-between text-xs text-gray-700 dark:text-gray-300">
                      <span>
                        {startIndex + 1}-{Math.min(endIndex, filteredOOTOH.length)} of {filteredOOTOH.length}
                      </span>
                      <select
                        value={itemsPerPage}
                        onChange={(e) => {
