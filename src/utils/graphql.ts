...
export async function graphqlRequest<T = any>(
  ...
  try {
    ...
    // Use AUTH_GRAPHQL_URL for auth operations, GRAPHQL_URL for everything else
    const graphqlUrl = useAuthUrl
      ? AUTH_CONFIG.AUTH_GRAPHQL_URL
      : AUTH_CONFIG.GRAPHQL_URL;
      ...
      if (response.status === 401 && !customToken) {
        ...
        if (authState.authenticated && authState.user?.refresh_token) {
          ...
          if (newToken && newToken !== token) {
            ...
            return graphqlRequest(query, variables, newToken, useAuthUrl);
          } else {
            console.log("❌ Token refresh failed or returned same token");
          }
        } else {
          console.log(
            "❌ Cannot refresh token - user not authenticated or no refresh token"
          );
        }
      }
