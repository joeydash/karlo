...
const AttendanceModal: React.FC<AttendanceModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen && currentOrganization?.id && selectedDate) {
      fetchAttendanceForDate(currentOrganization.id, selectedDate);
    }
  }, [isOpen, currentOrganization?.id, selectedDate, fetchAttendanceForDate]);

  // Handle date change
  const handleDateChange = (newDate: string) => {
    setSelectedDate(newDate);
    if (currentOrganization?.id) {
      fetchAttendanceForDate(currentOrganization.id, newDate);
    }
  };
  const handleMemberToggle = async (userId: string) => {
    ...
    if (!currentOrganization?.id) {
      ...
      showWarning("Error", "Organization not found");
      return;
    }
  ...
  const confirmClearAll = async () => {
    ...
    const result = await clearAttendance(currentOrganization.id, selectedDate);
    setShowClearConfirmation(false);

    if (result.success) {
      showSuccess(
        "Attendance Cleared",
        `Successfully cleared attendance for ${clearedCount} member${
          clearedCount !== 1 ? "s" : ""
        }`
      );
    } else {
      showWarning(
        "Clear failed",
        result.message || "Failed to clear attendance"
      );
    }
  };
  ...
  const filteredMembers = members.filter((member) => {
    const matchesSearch = member.auth_fullname.fullname
      .toLowerCase()
    ...
    >
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              >
                ...
                <span>•</span>
                <span>
                  {getMarkedMemberIds().size} of {members.length} members marked
                </span>
              </p>
            </div>
          </div>
          ...
          {isLoading || isAttendanceLoading ? (
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap justify-center pb-2 gap-2">
                {filterButtons.map((filter) => (
                  <button
                    key={filter.key}
                    onClick={() =>
                      setAttendanceFilter(filter.key as FilterType)
                    }
              ...
              <div>
                <div
                  ref={memberListRef}
                  className="space-y-2 max-h-60 overflow-y-auto attendance-modal-scroll pr-2"
                >
                    ...
                    return (
                      <button
                        ...
                        aria-label={
                          isCurrentUserAdmin
                            ? `${
                                isMarkedForAttendance ? "Unmark" : "Mark"
                              } ${displayName} as ${
                                isMarkedForAttendance ? "absent" : "present"
                              }`
                            : `${displayName} is ${
                                isMarkedForAttendance ? "present" : "absent"
                              } (View only)`
                        }
                      >
                        <div className="flex items-center space-x-3">
                ...
                {isCurrentUserAdmin && (
                  <div className="bg-blue-50 dark:bg-blue-900/20 mt-5 rounded-lg p-3 border border-blue-200 dark:border-blue-800">
                    <p className="text-xs text-blue-700 dark:text-blue-300 text-center">
                      <span className="font-medium">Navigation:</span> Use arrow
                      keys to navigate • Press{" "}
              ...
              {!searchTerm &&
                ...
                !isLoading && (
                  <div className="text-center py-8">
                    {activeFilterButton && (
                      <activeFilterButton.icon className="h-12 w-12 text-gray-300 mx-auto mb-3" />
                    )}
                    <p className="text-gray-500 dark:text-gray-400">
      ...
      {sortedFilteredMembers.length > 0 && (
        <div className="sr-only" aria-live="polite">
          ...
          {isCurrentUserAdmin ? ", Enter to mark/unmark" : " (view only)"},
          Escape to return to search
          {selectedMemberIndex >= 0 && (
            <>
              . Currently highlighting:{" "}
              {sortedFilteredMembers[selectedMemberIndex]?.auth_fullname
                ?.fullname || "Unknown"}
            </>
          )}
        </div>
      ...
      <ConfirmationModal
        ...
        message={`Are you sure you want to clear all attendance markings? This will unmark all ${
          getMarkedMemberIds().size
        } member${
          getMarkedMemberIds().size !== 1 ? "s" : ""
