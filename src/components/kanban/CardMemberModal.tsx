...
const CardMemberModal: React.FC<CardMemberModalProps> = ({ isOpen, onClose, cardId, cardTitle }) => {
  ...
  const { getCardMembers, getOrganizationMembers, addCardMember, removeCardMember } = useKanban();
  ...
  const handleAddMember = async (userId: string) => {
    ...
    const memberToAdd = organizationMembers.find(member => member.user_id === userId);
    if (!memberToAdd) {
    ...
    if (result.success) {
      ...
      setCardMembers(prev => prev.filter(member => member.user_id !== userId));
  ...
  }, [isOpen, isLoading, selectedMemberIndex, totalNavigableItems, availableMembers, handleAddMember]);
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-100 dark:border-gray-700">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900 dark:text-white" id="member-modal-title">Card Members</h2>
              <p className="text-sm text-gray-600 dark:text-gray-300 truncate max-w-48" id="member-modal-description">{cardTitle}</p>
            </div>
          </div>
          <button
            onClick={handleClose}
        ...
        <div className="p-6">
          {isLoading ? (
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {Array.from({ length: 2 }).map((_, index) => (
                    <div key={index} className="w-10 h-10 bg-gradient-to-r from-gray-200 via-gray-300 to-gray-200 bg-[length:200%_100%] animate-[shimmer_1.5s_ease-in-out_infinite] rounded-full"></div>
                  ))}
                </div>
              </div>
            ...
            <div className="space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {cardMembers.map((member) => (
                      <div
                        ...
                        title={`${member.authFullnameByUserId.fullname} - Assigned ${new Date(member.assigned_at).toLocaleDateString()}`}
                      >
                        <button
              ...
              <div>
                ...
                <div className="relative mb-4">
                  ...
                  <input
                    ...
                    aria-describedby={filteredOrganizationMembers.length > 0 ? "search-results-count" : undefined}
                    autoFocus
                  />
                </div>
                ...
                <div ref={memberListRef} className="space-y-2 max-h-60 overflow-y-auto">
              ...
              {!searchTerm && organizationMembers.filter(member => !isUserAssigned(member.user_id)).length === 0 && !isLoading && (
                <div className="text-center py-8">
                  <User className="h-12 w-12 text-gray-300 mx-auto mb-3" />
                  <p className="text-gray-500 dark:text-gray-400">All organization members are already assigned</p>
                </div>
              )}
            </div>
          )}
      ...
      {totalNavigableItems > 0 && (
        <div className="sr-only" aria-live="polite">
          Use arrow keys to navigate, Enter to select, Escape to return to search
          {selectedMemberIndex >= 0 && (
            <>
              . Currently highlighting: {availableMembers[selectedMemberIndex]?.auth_fullname?.fullname || 'Unknown'}
            </>
          )}
        </div>
      )}
