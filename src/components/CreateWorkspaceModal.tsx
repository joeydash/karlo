...
const CreateWorkspaceModal: React.FC<CreateWorkspaceModalProps> = ({ isOpen, onClose }) => {
  ...
  const { createOrganizationWithUser, isLoading } = useOrganization();
  const { user } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user?.id) {
      setErrors({ display_name: 'User not authenticated' });
      return;
    }
    ...
    if (result.success) {
      ...
      setFormData({ name: '', display_name: '', description: '', website: '', logo_url: '' });
      setErrors({});
      onClose();
    } else {
      showError(result.message || 'Failed to create workspace');
    }
  };

  const generateSlugFromName = (displayName: string) => {
    return displayName
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '_')
      .replace(/^_|_$/g, '');
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-100 dark:border-gray-700">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 dark:text-gray-300">Set up your new workspace</p>
            </div>
          </div>
          <button
            onClick={handleClose}
