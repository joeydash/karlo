...
const CreateHolidayModal: React.FC<CreateHolidayModalProps> = ({ isOpen, onClose }) => {
  ...
  const { createHoliday, isLoading } = useHoliday();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!currentOrganization?.id) {
      setErrors({ reason: 'No organization selected' });
      return;
    }
    ...
    if (result.success) {
      ...
      setFormData({ date: '', reason: '', type: 'company' });
      setErrors({});
      onClose();
    } else {
      showError(result.message || 'Failed to create holiday');
      setErrors({ reason: result.message || 'Failed to create holiday' });
    }
  };
        ...
        <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-100 dark:border-gray-700">
          <div className="flex items-center space-x-2 sm:space-x-3 min-w-0 flex-1">
            ...
            <div className="min-w-0">
              <h2 className="text-base sm:text-xl font-bold text-gray-900 dark:text-white truncate">Add Holiday</h2>
              <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-300 hidden sm:block">Create a new holiday for your organization</p>
            </div>
          </div>
          <button
            onClick={handleClose}
