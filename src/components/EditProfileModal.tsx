...
import { X, User, Mail, Loader2, AlertCircle, CheckCircle, LogOut, Smartphone, Car as IdCard, CreditCard, Upload, Image as ImageIcon, Edit, Camera, Trash2, Clock, MapPin, Award, FileText, IndianRupee } from 'lucide-react';
...
const EditProfileModal: React.FC<EditProfileModalProps> = ({ isOpen, onClose }) => {
  ...
  const { user, updateProfile, isUpdatingProfile, logout, isLoggingOut } = useAuth();
  const { showSuccess, showError } = useToast();
  const { currentOrganization } = useOrganization();
  const { members } = useMember();
  const { addresses, isLoading: isLoadingAddresses, addAddress, updateAddress, deleteAddress } = useAddress();
  ...
  const [selectedMemberForOffer, setSelectedMemberForOffer] = useState<Member | null>(null);
  const bankDetailsRef = React.useRef<{ loadBankDetails: () => Promise<void> }>(null);
        ...
        <div className="relative max-w-2xl w-full h-[95vh] sm:h-[90vh] perspective-1000 overflow-hidden">
          ...
          <div className={`absolute inset-0 w-full h-full backface-hidden ${isFlipped ? 'pointer-events-none' : 'pointer-events-auto'}`}>
            <div className="bg-white dark:bg-gray-800 rounded-2xl sm:rounded-3xl shadow-2xl w-full h-full flex flex-col">
                ...
                <div className="flex items-center space-x-2 sm:space-x-3 min-w-0 flex-1">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-300 truncate">
                      {isEditMode ? 'Update your information' : 'View your account details'}
                    </p>
                  </div>
                </div>
              ...
              {/* Content */}
              <div className="flex-1 overflow-y-auto profile-modal-scroll" style={{ overflowY: 'auto', height: 'auto' }}>
                {!isEditMode ? (
                    ...
                    <div className="flex flex-col items-center text-center space-y-3 sm:space-y-4">
                      <div className="relative group">
                        <div className="w-24 h-24 sm:w-32 sm:h-32 rounded-full border-4 border-white dark:border-gray-700 shadow-xl overflow-hidden flex items-center justify-center bg-gradient-to-br from-blue-500 to-purple-600">
                          {formData.dp ? (
                            <img src={formData.dp} alt="Profile" className="w-full h-full object-cover" />
                          ) : (
                          ...
                          <div className="flex items-center justify-between gap-2">
                            <div className="flex items-center space-x-2 sm:space-x-3 min-w-0">
                              ...
                              <div className="min-w-0">
                                ...
                                <p className="text-base sm:text-lg font-semibold font-mono text-gray-900 dark:text-white">{user.phone}</p>
                              </div>
                            </div>
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-lg sm:rounded-xl p-3 sm:p-4 shadow-sm">
                          <div className="flex items-center space-x-2 sm:space-x-3">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-xs sm:text-sm font-mono text-gray-900 dark:text-white break-all">{user.id}</p>
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-xl sm:rounded-2xl p-4 sm:p-6">
                      ...
                      <div className="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-6">
                        ...
                        <div className="flex-1 w-full">
                          ...
                          <p className="mt-2 text-xs text-gray-500 dark:text-gray-400 text-center sm:text-left">Maximum file size: 10MB. Images will be compressed for optimal performance.</p>
                        </div>
                      </div>
                    </div>
          ...
          <div className={`absolute inset-0 w-full h-full backface-hidden rotate-y-180 ${isFlipped ? 'pointer-events-auto' : 'pointer-events-none'}`}>
            <div className="bg-white dark:bg-gray-800 rounded-2xl sm:rounded-3xl shadow-2xl w-full h-full">
              <BankDetailsForm ref={bankDetailsRef} onBack={handleBackToProfile} onClose={onClose} />
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="flex items-start space-x-3 sm:space-x-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm sm:text-base text-gray-600 dark:text-gray-300">
                  Are you sure you want to remove your profile picture? It will be replaced with the default image.
                </p>
              </div>
            </div>
